    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-body">
          <div class="container-fluid d-flex flex-column">
            <div class="row" v-if="$route.name == 'Photos'">
              <div class="col-md-6">
                <img src="" class="align-self-center border border-3 p-2 rounded-circle new-photo" alt="" loading="lazy" />
              </div>
              <div class="col-md-6">
                <div class="mb-3 text-center">
                  <input class="form-control form-control-sm mt-4" type="file" @change="getImage" />
                  <small>請上傳正確圖片</small>
                </div>
              </div>
            </div>
            <form class="needs-validation mt-4" novalidate v-if="$route.name == 'Coupons'">
              <label for="coupon-id" class="form-label fw-bolder">優惠券代碼</label>
              <input type="text" class="form-control" id="coupon-id" v-model="code" disabled />
              <div class="my-2">
                <label for="coupon-type" class="form-label fw-bolder">適用玩具類型</label>
                <select class="form-select rounded-0" id="coupon-type" required v-model="title">
              ...
              <div class="my-2">
                ...
                <input type="text" class="form-control" id="coupon-now-price" required v-model="percent" />
                <div class="invalid-feedback">請給予正確數字(80~95)</div>
              </div>
              <div class="my-2">
                <label for="coupon-date-end" class="form-label fw-bolder">到期日</label>
                <input type="datetime-local" class="form-control" id="coupon-now-price" @change="changeDeadline" required />
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-info rounded-0" @click.prevent="add" data-bs-dismiss="modal">儲存</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup(props) {
    ...
    function changeDeadline() {
      currentEditCoupon.deadline_at = new Date(document.querySelector('input[type="datetime-local"]').value)
